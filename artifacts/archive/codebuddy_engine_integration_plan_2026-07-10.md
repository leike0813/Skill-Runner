...
| model observations | --help 的 --model 段可辅助维护快照 | 运行时使用引擎内静态 manifest，不执行探测 |
...
- provider_id 是网络环境、credential、配置目录、session resume 和静态模型条目的统一路由键。
...
- 模型通过引擎内 provider-qualified 静态 manifest 获取，通用 ModelRegistry 负责解析。
...
- 运行时模型探测、provider 模型缓存与 probe 证据。
...
4. start、resume 使用相同 provider config directory；模型列表不读取该目录。
...
| Model manifest | <engine>/models/manifest.json | repository | 通用静态 manifest SSOT |
| Model snapshot | <engine>/models/models_0.0.0.json | repository | provider-qualified 固定快照 |
...
| models/manifest.json + snapshot | generic ModelRegistry input | provider-qualified 静态模型 SSOT |
...
| server/engines/codebuddy/schemas/release_gate.schema.json | CodeBuddy 专属发布门禁；不放入全局 contracts |
...
| server/engines/codebuddy/models/manifest.json | 静态快照索引 |
| server/engines/codebuddy/models/models_0.0.0.json | provider-qualified 模型快照 |
| server/engines/codebuddy/schemas/release_gate.schema.json | CodeBuddy 发布门禁合同 |
...
- runtime handler 接入 oauth_proxy/auth_code_or_url；成功后写 vault，不触发模型探测。
...
#### CB-2.3 Provider-qualified 静态模型 manifest

依赖：adapter profile 和通用 ModelRegistry 合同。

实施：

- adapter profile 使用 `mode=manifest` 并指向引擎内 `models/manifest.json`。
- 固定快照为两个 provider 生成 provider-qualified record，同名原始模型不冲突。
- 删除 CodeBuddy runtime catalog、probe cache、LKG 与认证后刷新接线。
- provider/model 错配由通用 ModelRegistry fail closed；model omission 保持有效。

验收：同名模型不冲突、provider 过滤正确、未安装或未登录时模型列表仍可读取，且不启动 CLI。
...
| static manifest/provider filter | 是 | models API | 否 |
...
### 12.2 核心命令
    ...
    conda run --no-capture-output -n DataProcessing python -u -m pytest tests/unit/test_codebuddy_adapter.py tests/unit/test_codebuddy_credential_store.py tests/unit/test_codebuddy_release_contracts.py tests/unit/test_model_registry.py tests/unit/test_engine_adapter_registry.py tests/unit/test_engine_auth_bootstrap.py tests/unit/test_mcp_config_governance.py tests/unit/test_runtime_parser_capability_contract.py tests/unit/test_agent_cli_manager.py tests/unit/test_ui_routes.py
