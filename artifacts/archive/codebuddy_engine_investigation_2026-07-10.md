...
### Model observations and repository contract
...
**Repository decision:** CodeBuddy follows Codex's pinned-manifest model. The observed lists inform the reviewed provider-qualified snapshot under `server/engines/codebuddy/models/`, but runtime code does not probe `--help`, persist probe output, or bind model visibility to credentials. Updating the supported list is an explicit repository change.
...
That change should define the engine adapter, static provider-qualified model manifest,
