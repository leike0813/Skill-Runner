...
@pytest.mark.asyncio
async def test_reply_interaction_sticky_profile_transitions_to_running(monkeypatch, temp_config_dirs):
    store, request_id = await _create_interactive_request(monkeypatch, temp_config_dirs)
    request_record = store.get_request(request_id)
    assert request_record is not None
    run_id = request_record["run_id"]
    ...
    store.set_interactive_profile(
        request_id,
        {"kind": "sticky_process", "reason": "probe_failed", "session_timeout_sec": 1200},
    )
    store.set_sticky_wait_runtime(
        request_id,
        "2099-01-01T00:00:00",
        {"run_id": run_id, "alive": True},
    )

    response = await jobs_router.reply_interaction(
        request_id,
        InteractionReplyRequest(interaction_id=7, response={"confirm": True}),
        BackgroundTasks(),
    )
    assert response.accepted is True
...
@pytest.mark.asyncio
async def test_reply_interaction_sticky_timeout_maps_error(monkeypatch, temp_config_dirs):
    store, request_id = await _create_interactive_request(monkeypatch, temp_config_dirs)
    request_record = store.get_request(request_id)
    assert request_record is not None
    run_id = request_record["run_id"]
    ...
    store.set_interactive_profile(
        request_id,
        {"kind": "sticky_process", "reason": "probe_failed", "session_timeout_sec": 1},
    )
    store.set_sticky_wait_runtime(
        request_id,
        "2000-01-01T00:00:00",
    ...
    with pytest.raises(HTTPException) as excinfo:
        await jobs_router.reply_interaction(
            request_id,
            InteractionReplyRequest(interaction_id=8, response={"detail": "x"}),
            BackgroundTasks(),
        )
    assert excinfo.value.status_code == 409
    status_payload = json.loads((Path(config.SYSTEM.RUNS_DIR) / run_id / "status.json").read_text(encoding="utf-8"))
    assert status_payload["status"] == RunStatus.FAILED.value
    assert status_payload["error"]["code"] == "INTERACTION_WAIT_TIMEOUT"


@pytest.mark.asyncio
async def test_reply_interaction_sticky_process_lost_maps_error(monkeypatch, temp_config_dirs):
    store, request_id = await _create_interactive_request(monkeypatch, temp_config_dirs)
    request_record = store.get_request(request_id)
    assert request_record is not None
    run_id = request_record["run_id"]
    ...
    store.set_sticky_wait_runtime(
        request_id,
        "2099-01-01T00:00:00",
        {"run_id": run_id, "alive": False},
    )

    with pytest.raises(HTTPException) as excinfo:
        await jobs_router.reply_interaction(
            request_id,
            InteractionReplyRequest(interaction_id=9, response={"detail": "x"}),
            BackgroundTasks(),
        )
    assert excinfo.value.status_code == 409
    status_payload = json.loads((Path(config.SYSTEM.RUNS_DIR) / run_id / "status.json").read_text(encoding="utf-8"))
    assert status_payload["status"] == RunStatus.FAILED.value
    assert status_payload["error"]["code"] == "INTERACTION_PROCESS_LOST"
