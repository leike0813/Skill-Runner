...
def test_callback_state_store_unregister_keeps_consumed_marker():
    store = CallbackStateStore()
    store.register(channel="qwen", state="state-x", session_id="s-x")
    store.consume(channel="qwen", state="state-x")
    assert store.is_consumed(channel="qwen", state="state-x") is True

    store.unregister(channel="qwen", state="state-x")
    assert store.is_consumed(channel="qwen", state="state-x") is True
    assert store.resolve_session_id(channel="qwen", state="state-x") is None
