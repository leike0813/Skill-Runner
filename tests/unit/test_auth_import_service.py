...
def test_get_import_spec_gemini_is_unsupported(tmp_path: Path) -> None:
    service = _build_service(tmp_path)

    with pytest.raises(AuthImportError, match="Unsupported engine"):
        service.get_import_spec(engine="gemini")
...
def test_import_auth_files_gemini_is_unsupported(tmp_path: Path) -> None:
    service = _build_service(tmp_path)
