...
def test_build_reply_prompt_preserves_strings_and_serializes_objects():
    orchestrator = JobOrchestrator()

    assert orchestrator._build_reply_prompt("继续执行") == "继续执行"
    assert orchestrator._build_reply_prompt({"confirm": True}) == '{"confirm": true}'


def test_error_from_exception_maps_protocol_schema_violation():
    orchestrator = JobOrchestrator()

    error = orchestrator._error_from_exception(RuntimeError("SESSION_RESUME_FAILED: missing thread.started"))
    assert error == {
        "code": "SESSION_RESUME_FAILED",
        "message": "SESSION_RESUME_FAILED: missing thread.started",
    }
