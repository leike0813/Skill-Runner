...
### 3.4 `output_repair.<attempt>.jsonl`

output convergence 的 attempt-local internal-round 历史。
...
- 该文件是 phase 3B target，phase 3A 先定义合同，当前实现可仍然缺失
- 其记录必须服务 `attempt + internal_round` 双层治理模型
- canonical 字段包括 `attempt_number`、`internal_round_index`、`repair_stage`、`candidate_source`、`executor`、`legacy_fallback_target`

### 3.5 `fcmp_events.<attempt>.jsonl`

对外 FCMP 事件流的持久化切片。

### 3.6 `stdout.<attempt>.log` / `stderr.<attempt>.log` / `pty-output.<attempt>.log`

attempt 级原始执行输出。

### 3.7 `service.run.log`（及轮转分片）
...
### 3.9 `parser_diagnostics.<attempt>.jsonl`
...
### 3.10 `protocol_metrics.<attempt>.json`
