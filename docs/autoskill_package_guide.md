...
- `runtime.default_options` 是可选的 skill 默认 runtime options 声明：
  - 仅系统允许的 runtime option 键会生效（例如 `execution_mode`、`no_cache`、`interactive_auto_reply`、`interactive_reply_timeout_sec`、`hard_timeout_seconds`、`preamble_prompt`）；
...
- **interactive 模式**：允许受控的多轮交互（如向用户确认、展示中间结果并询问下一步），但交互点应受统一 JSON 合同约束。
...
- `PendingInteraction` 的目标来源是合法中间态 JSON 的投影；对外接口形状可保持不变。
- `max_attempt`（runner.json 可选字段，正整数 >= 1）仅作用于 interactive 模式，控制最大交互轮次。
- tool_use / tool 回显中的标记文本不参与完成判定。
- `<ASK_USER_YAML>` 与 soft completion 相关行为若仍在当前实现中出现，只属于 legacy / deprecated rollout 背景，不是目标合同。
...
- 问题：输出字段很多，但 artifact 没有显式声明。  
  建议：在 `output.schema.json` 中为文件字段加 `x-type: "artifact"` 和明确 `x-role`。
