...
#### Scenario: client retries an auth method selection after challenge creation
- **GIVEN** a run already has an active auth challenge
- **WHEN** the client submits the selected method with the matching `auth_session_id`
- **THEN** the backend MUST accept the request idempotently when the method also matches
- **AND** it MUST reject a mismatched session or method without replacing the active challenge
