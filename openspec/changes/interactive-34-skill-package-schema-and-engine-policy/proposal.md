...
- 新增 `runner.json.unsupport_engine` 字段用于声明不支持的引擎。
- 当 `engines` 与 `unsupport_engine` 同时存在时，系统必须校验两者无重复项。
- 定义并落实“有效引擎集合”语义：`effective_engines = (engines 或 系统全量支持引擎) - unsupport_engine`。
- 运行请求若命中不支持引擎（含落在 `unsupport_engine` 或不在 `effective_engines`）必须拒绝执行并返回错误。
...
- `skill-package-install`: 调整 `engines` 约束为可选，并引入 `unsupport_engine`/冲突检查/有效引擎集合语义。
- `ephemeral-skill-validation`: 与持久安装保持一致的 `engines` 可选与 `unsupport_engine` 校验规则。
