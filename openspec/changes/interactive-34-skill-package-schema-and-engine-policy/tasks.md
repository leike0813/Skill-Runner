...
- [x] 1.1 在 `server/assets/schemas/` 新增 skill package / runner manifest 合同 schema 文件（含 `engines`、`unsupport_engine`、`execution_modes` 等约束）
...
- [x] 2.1 实现统一 engine policy 解析：支持 `engines` 可选 + `unsupport_engine` 排除 + 无重叠校验
...
- [x] 3.3 扩展 management skill 响应字段，返回 `effective_engines` 以及原始 `engines`/`unsupport_engine`

## 4. 测试、文档与回归

- [x] 4.1 补充/更新单测：schema 驱动校验、`engines` 缺失默认语义、`unsupport_engine` 冲突与空集合拒绝
