...
- **WHEN** 同一个 `runner.json` 在 `engines`/`unsupport_engine` 字段违反合同
- **THEN** 安装与临时上传都拒绝该包
- **AND** 拒绝原因为同一类合同违规

### Requirement: runner engine 声明合同 MUST 支持“允许 + 排除”组合语义
`runner.json` 合同 MUST 支持 `engines`（可选）与 `unsupport_engine`（可选）联合声明，并满足：  
...
#### Scenario: 仅声明排除列表
- **WHEN** `runner.json` 省略 `engines` 且声明 `unsupport_engine`
- **THEN** 系统按“全量支持引擎减去排除列表”计算有效引擎集合
- **AND** 该包通过 engine 合同校验（前提是结果非空）

#### Scenario: 允许与排除声明重复
- **WHEN** `engines` 与 `unsupport_engine` 同时存在且包含重复 engine
