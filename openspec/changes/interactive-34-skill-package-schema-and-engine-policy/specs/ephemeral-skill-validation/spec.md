...
- **THEN** the system computes `effective_engines` from all system-supported engines minus `unsupport_engine`
...
- **WHEN** temporary `runner.json` contains the same engine in both `engines` and `unsupport_engine`
