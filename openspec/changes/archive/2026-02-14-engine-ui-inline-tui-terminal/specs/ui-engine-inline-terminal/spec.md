...
- **THEN** 系统在 managed 环境启动该引擎 CLI 的默认 TUI
- **AND** 页面内嵌终端显示该会话输出

#### Scenario: 旧独立页面下线
- **WHEN** 用户访问 `/ui/engines/auth-shell`
- **THEN** 系统返回 404
...
系统 MUST 在启动引擎 TUI 前完成 sandbox 可用性校验；若无法确认 sandbox 可用，必须拒绝启动。

#### Scenario: sandbox 可用
- **WHEN** 用户点击某引擎“在内嵌终端中启动 TUI”
- **AND** 该引擎 sandbox 校验通过
- **THEN** 系统启动会话并附加终端

#### Scenario: sandbox 不可用
- **WHEN** 用户点击某引擎“在内嵌终端中启动 TUI”
- **AND** 该引擎 sandbox 校验失败
- **THEN** 系统拒绝启动会话
- **AND** 返回明确错误原因（如 `sandbox_unavailable`）
- **AND** 不允许降级为非沙箱运行
