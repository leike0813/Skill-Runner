...
环境变量配置[​](#环境变量配置 "环境变量配置的直接链接")
--------------------------------

iFlow CLI 现在支持通过环境变量进行配置，**所有 `~/.iflow/settings.json` 中的配置项都可以通过 IFLOW\_ 前缀的环境变量设置**，避免与其他项目的环境变量冲突。

### 环境变量命名约定[​](#环境变量命名约定 "环境变量命名约定的直接链接")

#### IFLOW 前缀规则[​](#iflow-前缀规则 "IFLOW 前缀规则的直接链接")

**为了避免与其他项目的环境变量冲突，所有环境变量都必须使用 `IFLOW` 或 `iflow` 前缀。**

对于 `settings.json` 中的任何配置项，iFlow 支持以下 4 种环境变量命名约定（按优先级排序）：

1.  **IFLOW\_前缀的驼峰命名** - `IFLOW_` + settings.json 中的 key 名称（推荐）
2.  **IFLOW\_前缀的下划线命名** - `IFLOW_` + 大写下划线格式
3.  **iflow\_前缀的驼峰命名** - `iflow_` + settings.json 中的 key 名称
4.  **iflow\_前缀的下划线命名** - `iflow_` + 大写下划线格式

#### 命名示例[​](#命名示例 "命名示例的直接链接")



* settings.json 中的 key: apiKey
  * 1. IFLOW_驼峰: IFLOW_apiKey
  * 2. IFLOW_下划线: IFLOW_API_KEY
  * 3. iflow_驼峰: iflow_apiKey
  * 4. iflow_下划线: iflow_API_KEY
* settings.json 中的 key: baseUrl
  * 1. IFLOW_驼峰: IFLOW_baseUrl
  * 2. IFLOW_下划线: IFLOW_BASE_URL
  * 3. iflow_驼峰: iflow_baseUrl
  * 4. iflow_下划线: iflow_BASE_URL
* settings.json 中的 key: modelName
  * 1. IFLOW_驼峰: IFLOW_modelName
  * 2. IFLOW_下划线: IFLOW_MODEL_NAME
  * 3. iflow_驼峰: iflow_modelName
  * 4. iflow_下划线: iflow_MODEL_NAME
* settings.json 中的 key: vimMode
  * 1. IFLOW_驼峰: IFLOW_vimMode
  * 2. IFLOW_下划线: IFLOW_VIM_MODE
  * 3. iflow_驼峰: iflow_vimMode
  * 4. iflow_下划线: iflow_VIM_MODE
* settings.json 中的 key: showMemoryUsage
  * 1. IFLOW_驼峰: IFLOW_showMemoryUsage
  * 2. IFLOW_下划线: IFLOW_SHOW_MEMORY_USAGE
  * 3. iflow_驼峰: iflow_showMemoryUsage
  * 4. iflow_下划线: iflow_SHOW_MEMORY_USAGE


#### 支持的配置项[​](#支持的配置项 "支持的配置项的直接链接")
...
#### 方法 1: 使用 IFLOW\_前缀的驼峰命名（推荐）[​](#方法-1-使用-iflow_前缀的驼峰命名推荐 "方法 1: 使用 IFLOW_前缀的驼峰命名（推荐）的直接��链接")
...
#### 方法 2: 使用 IFLOW\_前缀的下划线命名[​](#方法-2-使用-iflow_前缀的下划线命名 "方法 2: 使用 IFLOW_前缀的下划线命名的直接链接")
...
#### 方法 3: 使用 iflow\_前缀（小写）[​](#方法-3-使用-iflow_前缀小写 "方法 3: 使用 iflow_前缀（小写）的直接链接")
...
#### 方法 4: 一行设置并启动[​](#方法-4-一行设置并启动 "方法 4: 一行设置并启动的直接链接")
...
#### 方法 5: 设置更多配置项[​](#方法-5-设置更多配置项 "方法 5: 设置更多配置项的直接链接")
...
#### 自定义 API 服务[​](#自定义-api-服务 "自定义 API 服务的直接链接")
...
#### 在 CI/CD 环境中使用[​](#在-cicd-环境中使用 "在 CI/CD 环境中使用的直接链接")
...
#### 从配置文件迁移到环境变量[​](#从配置文件迁移到环境变量 "从配置文件迁移到环境变量的直接链接")
...
### 项目中的 `.iflow` 目录[​](#项目中的-iflow-目录 "项目中的-iflow-目录的直接链接")

除了项目设置文件外，项目的 `.iflow` 目录还可以包含与 iFlow CLI 操作相关的其他项目特定文件，例如：

*   [自定义沙箱配置文件](#%E6%B2%99%E7%AE%B1)（如 `.iflow/sandbox-macos-custom.sb`、`.iflow/sandbox.Dockerfile`）。

### `settings.json` 中的可用设置：[​](#settingsjson-中的可用设置 "settingsjson-中的可用设置的直接链接")
...
*   **`sandbox`**（布尔值或字符串）：
    ...
    *   **描述：** 控制是否以及如何使用沙箱进行工具执行。如果设置为 `true`，iFlow CLI 使用预构建的 `iflow-cli-sandbox` Docker 镜像。更多信息请参见[沙箱](#%E6%B2%99%E7%AE%B1)。
    *   **默认值：** `false`
    *   **示例：** `"sandbox": "docker"`
*   **`mcpServers`**（对象）：
...
*   **`checkpointing`**（对象）：
    ...
    *   **描述：** 配置检查点功能，允许您保存和恢复对话和文件状态。更多详情请参见[检查点文档](https://platform.iflow.cn/cli/features/checkpointing)。
    *   **默认值：** `{"enabled": false}`
    *   **属性：**
...
*   **`telemetry`**（对象）
    ...
    *   **描述：** 配置 iFlow CLI 的日志记录和指标收集。更多信息请参见[遥测](https://platform.iflow.cn/cli/features/telemetry)。
...
*   **`approvalMode`**（字符串）
    ...
    *   **描述：** CLI 启动时默认模式，支持 yolo,plan,autoEdit,default 模式
    *   **默认值：** `"yolo"`
    *   **示例：**
*   **`language`**（字符串）
    
    *   **描述：** CLI 启动时默认语言，支持 zh-CN 和 en-US
    *   **默认值：** `"zh-CN"`
    *   **示例：**
...
*   **`accessibility`**（字符串）
    ...
    *   **描述：** 只支持 disableLoadingPhrases 选项，用于禁用加载短语的显示
    *   **默认值：** `false`
    *   **示例：**
...
*   **`toolSummarizationSettings`**（对象）
    
    *   **描述：** 用于控制是否对工具的输出进行摘要处理
    *   **默认值：** `空`
    *   **示例：**
...
*   **`hookManager`**（布尔值）
    ...
    *   **默认值：** `true` 启用
    *   **示例：**
*   **`enableBuildInTask`**（字符串）
    
    *   **描述：** 控制内置的通用目的任务代理的启用状态
    *   **默认值：** `"true"` 启用
    *   **示例：**
...
*   **`outputStyle`**（字符串）
    ...
    *   **描述：** 控制输出风格的参数，决定了 AI 助手与用户交互的方式和风格，可以配置为 Explanatory 和 Learning 风格
    *   **默认值：** `"Default"`
    *   **示例：**
...
*   **`hasIdeOnboardingBeenShown`**（对象）
    
    *   **描述：** 用于跟踪 IDE 插件自动安装功能的 onboarding 状态，确保自动安装提示只显示一次
    *   **默认值：** `空`
    *   **示例：**
...
*   **`customThemes`**（对象）
    ...
    *   **描述：** 用于定义自定义主题的参数，允许用户创建个性化的颜色方案
    *   **默认值：** `空`
    *   **示例：**
...
*   **`autoConfigureMaxOldSpaceSize`**（布尔值）
    
    *   **描述：** 用于控制是否自动配置 Node.js V8 引擎内存限制，为 true 会自动调整内存限制
    *   **默认值：** `undefined`
    *   **示例：**
...
*   **`hideWindowTitle`**（布尔值）
    ...
    *   **描述：** 控制是否在 CLI 运行时设置终端窗口标题
    *   **默认值：** `undefined`
    *   **示例：**
*   **`hideTips`**（布尔值）
    
    *   **描述：** 控制是否在 CLI 界面底部显示帮助提示
    *   **默认值：** `false`
    *   **示例：**
*   **`hideBanner`**（布尔值）
    
    *   **描述：** 控制是否在 CLI 启动时显示 ASCII 艺术 logo 横幅
    *   **默认值：** `false`
    *   **示例：**
*   **`lightWeightPlan`**（布尔值）
...
*   **`hooks`**（对象）
    ...
    *   **描述：** 配置钩子系统的参数，允许用户在特定事件发生时执行自定义命令
    *   **默认值：** `空`
    *   **示例：**
...
*   **`errorLog`**（对象）
    ...
    *   **描述：** 配置错误日志记录功能的参数 enabled-是否启用错误日志功能 maxInMemoryErrors-内存中保存的最大错误数量 enableFileLogging-是否启用文件日志记录
    *   **默认值：** `空`
    *   **示例：**
...
### `settings.json` 示例：[​](#settingsjson-示例 "settingsjson-示例的直接链接")
...
Shell 历史记录[​](#shell-历史记录 "Shell 历史记录的直接链接")
--------------------------------------------
...
*   **`--telemetry`**：
    *   启用[可观测性](https://platform.iflow.cn/cli/features/telemetry)。
*   **`--telemetry-target`**：
    *   设置可观测性目标。更多信息请参见[可观测性](https://platform.iflow.cn/cli/features/telemetry)。
*   **`--telemetry-otlp-endpoint`**：
    *   设置可观测性的 OTLP 端点。更多信息请参见[可观测性](https://platform.iflow.cn/cli/features/telemetry)。
*   **`--telemetry-log-prompts`**：
    *   启用可观测性的提示日志记录。更多信息请参见[可观测性](https://platform.iflow.cn/cli/features/telemetry)。
*   **`--checkpointing`**：
    *   启用[检查点](https://platform.iflow.cn/cli/features/checkpointing)。
...
*   **`--version`**：
    *   显示 CLI 的版本。
*   **`--resume`**（**`-r`**）：
    *   显示历史对话列表，让用户选择要恢复的对话会话。
    *   示例：`iflow --resume` 或 `iflow -r`
*   **`--continue`**（**`-c`**）：
    *   直接继续最近一次的对话会话，无需选择。
    *   示例：`iflow --continue` 或 `iflow -c`

上下文文件（分层指令上下文）[​](#上下文文件分层指令上下文 "上下文文件（分层指令上下文）的直接链接")
------------------------------------------------------
...
### 上下文文件内容示例（如 `IFLOW.md`）[​](#上下文文件内容示例如-iflowmd "上下文文件内容示例如-iflowmd的直接链接")

以下是 TypeScript 项目根目录中上下文文件��能包含的概念性示例：
