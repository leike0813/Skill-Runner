...
*   `diff_style` - 控制差异渲染方式。`"auto"` 根据终端宽度自适应，`"stacked"` ���终显示单列。
...
`small_model` 选项为标题生成等���量级任务配置单独的模型。默认情况下，如果您的提供商有更便宜的模型可用，OpenCode 会尝试使用该模型，否则会回退到您的主模型。
...
您还可以配置[本地模型](about:/docs/models#local)。[了解更多](https://opencode.ai/docs/models)。
...
[了解更多关于 Amazon Bedrock 配置的信息](about:/docs/providers#amazon-bedrock)。
