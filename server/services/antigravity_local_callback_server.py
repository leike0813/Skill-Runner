...
class AntigravityLocalCallbackServer:
    def __init__(
        self,
    ...
    def start(self) -> bool:
        with self._lock:
            ...
            class CallbackHandler(BaseHTTPRequestHandler):
                def do_GET(self) -> None:  # noqa: N802
                    ...
                    try:
                        ...
                        logger.exception("Antigravity local callback handling failed: %s", exc)
                        self._respond(
                            status=500,
                            title="OAuth callback failed",
                            message=str(exc),
            ...
            try:
                ...
                logger.warning(
                    "Antigravity local callback server unavailable on %s:%s: %s",
                    self._host,
                    self._port,
                    exc,
                )
            ...
            thread = Thread(target=server.serve_forever, daemon=True, name="antigravity-local-callback")
            thread.start()
            self._server = server
            self._thread = thread
            self._bound_port = int(server.server_port)
            logger.info(
                "Antigravity local callback server started at http://%s:%s/oauth-callback",
                self._host,
                self._bound_port,
            )
            return True
    ...
    def endpoint(self) -> str:
        return f"http://{self._host}:{self._bound_port}/oauth-callback"
