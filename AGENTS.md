...
|---|---|---|---|
| Session Canonical Statechart | `server/contracts/invariants/session_fcmp_invariants.yaml`（`canonical` / `transitions`） | `docs/session_runtime_statechart_ssot.md` | `server/runtime/session/statechart.py`、`server/services/orchestration/job_orchestrator.py` |
| FCMP 单流事件语义与时序 | `server/contracts/invariants/session_fcmp_invariants.yaml`（`fcmp_mapping` / `ordering_rules`） | `docs/session_event_flow_sequence_fcmp.md`、`docs/runtime_stream_protocol.md` | `server/runtime/protocol/event_protocol.py`、`server/runtime/observability/run_observability.py`、`server/runtime/protocol/factories.py`、`server/models.py` |
| 运行时事件/命令 Schema 合同 | `server/contracts/schemas/runtime_contract.schema.json` | `docs/runtime_event_schema_contract.md` | `server/runtime/protocol/schema_registry.py`、`server/runtime/protocol/event_protocol.py`、`server/services/orchestration/run_store.py`、`server/runtime/observability/run_observability.py` |
| 不变量守护与模型验证 | `server/contracts/invariants/session_fcmp_invariants.yaml` | `docs/session_runtime_statechart_ssot.md`（Canonical Invariants） | `tests/common/session_invariant_contract.py`、`tests/unit/test_session_invariant_contract.py`、`tests/unit/test_session_state_model_properties.py`、`tests/unit/test_fcmp_mapping_properties.py`、`tests/unit/test_protocol_state_alignment.py` |
...
### 禁止项
- 禁止在业务层直接手拼核心 FCMP/RASP payload（必须走 `server/runtime/protocol/factories.py`）。
...
## SSOT 漂移 PR 自检清单
1. 是否出现了新的 state/event 名称但未更新 `server/contracts/invariants/session_fcmp_invariants.yaml`？
